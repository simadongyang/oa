...
<html>
	...
	<div id="organizeIndex">
		<!-- 标题栏 -->
		<div class="main-title">
			<h2>部门列表</h2>
		</div>
		<div class="toolbar">
			<!--<a class="ui blue button" href="<?php echo U('Organize/add');?>">新 增</a>-->
			<button class="ui blue button ajax-post" url="<?php echo U('Organize/changeStatus',array('method'=>'resumeUser'));?>" target-form="ids">启 用</button>
			<button class="ui blue button ajax-post" url="<?php echo U('Organize/changeStatus',array('method'=>'forbidUser'));?>" target-form="ids">禁 用</button>
			<button class="ui blue button ajax-post confirm" url="<?php echo U('Organize/changeStatus',array('method'=>'deleteUser'));?>" target-form="ids">删 除</button>
		</div>
		<!-- 数据列表 -->
		<table class="ui celled table">
			...
			<tbody>
				<?php if(!empty($department)): if(is_array($department)): $i = 0; $__LIST__ = $department;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
							<td><input class="ids" type="checkbox" name="id[]" value="<?php echo ($vo["did"]); ?>" /></td>
							<td><?php echo ($vo["did"]); ?> </td>
							<td>
								<?php echo ($vo["dname"]); ?>
							</td>
							<td>
								<?php echo ($vo["personname"]); ?>
							</td>
							<td>
								<a href="<?php echo U('Organize/add?did='.$vo['did']);?>">编辑</a>
								<?php if(($vo["status"]) == "1"): ?><a href="<?php echo U('Organize/changeStatus?method=forbidUser&id='.$vo['did']);?>" class="ajax-get">禁用</a>
									<?php else: ?>
									<a href="<?php echo U('Organize/changeStatus?method=resumeUser&id='.$vo['did']);?>" class="ajax-get">启用</a><?php endif; ?>
								<a href="<?php echo U('Organize/changeStatus?method=deleteUser&id='.$vo['did']);?>" class="confirm ajax-get">删除</a>
							</td>
	...
	<script type="text/javascript">
		//导航高亮
		highlight_subnav('<?php echo U('Organize/index');?>');
	</script>
