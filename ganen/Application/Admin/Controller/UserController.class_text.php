    ...
    public function add($username = '', $password = '', $repassword = '',$criticalname='', $email = ''){
        ...
        //显示员工的岗位及薪资信息
           //查询员工部门岗位信息
            ...
            $findds=M('department_station')->where($where)->order('dsid desc')->find();
           
            $this->assign('findds',$findds);

            //根据员工所在部门岗位id查询现在是否有所属项目
            
            $where=array('uid'=>$id,'status'=>0,'dsid'=>$findds['dsid']);
            ...
            if($sel){
                foreach($sel as $k=>&$v){
                    ...
                    $sel[$k]['sid']=$findds['sid']; 
                    $sel[$k]['did']=$findds['did']; 
                }
            ...
            if($arr['savestatus']=='岗位'){
                ...
                if($arr['uid']){
                    ...
                    if($findst['isstaff']==1 && $arr['panduan']==1){
                        ...
                        $resul=M('department_station')->add($arr);
                        if($resul){
